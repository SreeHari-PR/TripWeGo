...
const ChatWindow = ({ chat, otherusername, currentUserId, onSendMessage }) => {
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-100">
        {chat && chat.messages.length > 0 ? (
          chat.messages.map((message, index) => (
            ...
            >
              <div
                className={`max-w-[70%] rounded-2xl p-4 ${message.sender === currentUserId
                    ? 'bg-indigo-500 text-white'
                    : 'bg-white shadow-md'}`}
              >
                <p className="text-sm leading-relaxed">{message.content}</p>
                <p className={`text-xs mt-2 ${message.sender === currentUserId ? 'text-indigo-200' : 'text-gray-500'}`}>
